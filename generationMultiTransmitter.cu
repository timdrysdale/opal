...
//Ray sphere generation for multiple transmitters
...
RT_PROGRAM void genRayAndReflectionsFromSphereIndex()
{
	...
	//3D kernel launch [elevation, azimuth, transmitter]	

	uint2 idx = make_uint2(launchIndex.x, launchIndex.y); //[elevation, azimuth]
	uint tindex=launchIndex.z;
	...
	const Transmitter my_tx=tx_origin[tindex];
	//const Transmitter my_tx = {make_float3(0.0f,0.0f,0.0f), make_float3(0.0f,0.0f,0.0f), 0};
	float3 origin = my_tx.origin; //Transmitter position
	...
	rayPayload.polarization = my_tx.polarization;
