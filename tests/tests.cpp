...
namespace opal {
	...
	std::unique_ptr<OpalSceneManager> addRemoveReceivers(std::unique_ptr<OpalSceneManager> sceneManager) {
		try {
			//Horizontal plane
			std::vector<int> planeind = Util::loadTrianglesFromFile("meshes/tri.txt");
			std::vector<optix::float3> planever = Util::Util::loadVerticesFromFile("meshes/vert.txt");
	...
	std::unique_ptr<OpalSceneManager> moveReceivers(std::unique_ptr<OpalSceneManager> sceneManager) {
		try {
			//Horizontal plane
			std::vector<int> planeind = Util::loadTrianglesFromFile("D:\\tri.txt");
			std::vector<optix::float3> planever = Util::loadVerticesFromFile("D:\\vert.txt");
			//std::cout << "indices=" << planeind.size() << "vertices=" << planever.size() << std::endl;
			Matrix4x4 tm;
			tm.setRow(0, make_float4(10.0f, 0, 0, 0.0f));
			tm.setRow(1, make_float4(0, 1, 0, 0.0f));
	...
	std::unique_ptr<OpalSceneManagerMultiTransmitter> crossingTestMulti(std::unique_ptr<OpalSceneManagerMultiTransmitter> sceneManager, bool print, bool subSteps) {
		...
		//Horizontal plane
		std::vector<int> planeind = Util::loadTrianglesFromFile("meshes/tri.txt");
		std::vector<float3> planever = Util::loadVerticesFromFile("meshes/vert.txt");
		//std::cout << "indices=" << planeind.size() << "vertices=" << planever.size() << std::endl;
	...
	std::unique_ptr<OpalSceneManager> crossingTest(std::unique_ptr<OpalSceneManager> sceneManager, bool print, bool subSteps) {
		...
		//Cubes
		std::vector<int> cubeind = Util::loadTrianglesFromFile("meshes/tricube.txt");
		std::vector<float3> cubevert = Util::loadVerticesFromFile("meshes/vertcube.txt");
		//std::cout << "indices=" << cubeind.size() << "vertices=" << cubevert.size() << std::endl;
		//Cube(4) NW
		Matrix4x4 tm;
		tm.setRow(0, make_float4(40.0f, 0, 0, -30.0f));
	...
	std::unique_ptr<OpalSceneManager> crossingTestAndVehicle(std::unique_ptr<OpalSceneManager> sceneManager) {
		...
		std::vector<int> bodyi = Util::loadTrianglesFromFile("meshes/CC_ME_Body_R4-i.txt");
		std::vector<float3> bodyv = Util::loadVerticesFromFile("meshes/CC_ME_Body_R4-v.txt");
		std::vector<int> wheeli = Util::loadTrianglesFromFile("meshes/CC_ME_Wheel_FL-i.txt");
		std::vector<float3> wheelv = Util::loadVerticesFromFile("meshes/CC_ME_Wheel_FL-v.txt");
		...
		wheeli = Util::loadTrianglesFromFile("meshes/CC_ME_Wheel_FR-i.txt");
		wheelv = Util::loadVerticesFromFile("meshes/CC_ME_Wheel_FR-v.txt");
		sceneManager->addMeshToGroup(0, static_cast<int>(wheelv.size()), wheelv.data(), static_cast<int>(wheeli.size()), wheeli.data(), emProp1);  //Call for each new mesh in the group

		wheeli = Util::loadTrianglesFromFile("meshes/CC_ME_Wheel_BL-i.txt");
		wheelv = Util::loadVerticesFromFile("meshes/CC_ME_Wheel_BL-v.txt");
		sceneManager->addMeshToGroup(0, static_cast<int>(wheelv.size()), wheelv.data(), static_cast<int>(wheeli.size()), wheeli.data(), emProp1);  //Call for each new mesh in the group
		wheeli = Util::loadTrianglesFromFile("meshes/CC_ME_Wheel_BR-i.txt");
		wheelv = Util::loadVerticesFromFile("meshes/CC_ME_Wheel_BR-v.txt");
		sceneManager->addMeshToGroup(0, static_cast<int>(wheelv.size()), wheelv.data(), static_cast<int>(wheeli.size()), wheeli.data(), emProp1);  //Call for each new mesh in the group
