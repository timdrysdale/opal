...
RT_PROGRAM void genRayAndReflectionsFromSphereIndex()
{
	...
	//2D kernel launch [elevation, azimuth]	
		...
		rayPayload.totalDistanceTillLastReflection = 0.0f;
...
RT_PROGRAM void closestHitReceiverFaceMin()
{
	...
	//Update ray data
	hitPayload.totalDistance += hit_attr.t; //Do not update total distance for receiver hit, only for hit with meshes, otherwise with multiple transmitters it fails, because it hits the radius, not the receiver position
