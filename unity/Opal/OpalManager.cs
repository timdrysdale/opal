...
namespace Opal
{
	public class OpalManager :Singleton<OpalManager>
	{
		...
		public bool holdReflections = false;
		public bool useInternalTracing = true;
		...
		{
			Debug.Log ("Initializing opal. multiTransmitter=" + multiTransmitter + " holdReflections=" + holdReflections);
			int r = OpalInterface.Init (frequency, useInternalTracing, holdReflections, multiTransmitter); 
			...
			if (r != 0) {
				throw new System.InvalidOperationException ("Error in SetDuplicateBlockSize:" + r);
			}
		...
		public virtual void Transmit (int id, float txPower, Vector3ToMarshal txPosition, Vector3ToMarshal polarization)
		{
			if (isInitialized) {
				
				int r = OpalInterface.Transmit (id, txPower, txPosition, polarization);
				if (r != 0) {
		...
		public void UnregisterTransmitter (Transmitter tx)
		{
			if (multiTransmitter) {
				if (isInitialized) {
		...
		public virtual void SendMeshToGroup (int id, Transform transform, Mesh mesh, MaterialEMProperties em)
		{
			...
			for (int j = 0; j < v.Length; j++) {
				...
				Vector3 aux = transform.InverseTransformPoint (transform.TransformPoint (v [j]));
		...
		public virtual void UpdateTransformInGroup (int id, Transform t)
		{
			...
			Debug.Log ("DynamicMesh Matrix of " + name + " is " + tm);
		...
		protected virtual void SendStaticMeshToOpal (Transform t, StaticMesh sm)
		{
			...
			//SaveMeshToFile (t.transform.name, vertices, indices, matrix, sm.GetEMProperties ());
